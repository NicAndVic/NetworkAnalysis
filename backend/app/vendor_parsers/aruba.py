...
def parse_aruba(content: str, fallback_hostname: str) -> ParsedDevice:
    ...
    model = ""
    if m := re.search(r"Model\s*:\s*(\S+)", content, re.IGNORECASE):
        model = m.group(1)

    mgmt = sorted(set(re.findall(r"\b(?:10|172|192)\.\d+\.\d+\.\d+\b", content)))[:3]
    roles = []
    if re.search(r"ip routing", raw, re.IGNORECASE):
        roles.append("L3")
    if re.search(r"dhcp-server", raw, re.IGNORECASE):
        roles.append("DHCP")

    stp_root = bool(re.search(r"Root this switch", blocks.get("show spanning-tree", ""), re.IGNORECASE))
    ...
    routes = [f"{p} {r}" for p, r in re.findall(r"^(S|C|O)\s+([^\n]+)", blocks.get("show ip route", ""), re.MULTILINE)]
    dhcp = [x.strip() for x in re.findall(r"Pool\s+([^\n]+)", blocks.get("show dhcp-server", "") + "\n" + content, re.IGNORECASE)]
